...
public class MessageRepository {
    ...
    public MessageRepository(DataSource dataSource, SessionFactory sessionFactory){
        ...
        this.sessionFactory = sessionFactory; // hibernate 자동 생성
    }

    public Message saveMessage(Message message){
        Session session = sessionFactory.openSession(); //세션 열기
        session.save(message);
        return message;
    }

    public List<Message> getMessage(){
        Session session = sessionFactory.getCurrentSession(); // table에 만든거 전부 가져옴
